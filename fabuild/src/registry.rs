...
impl FabuildRegistry {
    ...
    fn resolve_path(&self, name: &str) -> anyhow::Result<PathBuf> {
        let split = name
            .split('.')
    ...
    pub fn resolve_package(&self, name: &str) -> anyhow::Result<String> {
        let path = self.resolve_path(name)?.join("fabuild.toml");
